    ...
    <div>
      ...
      {chatVisible || minimized ? (
        ...
        <div className={`chatbot-container ${minimized ? 'minimized' : ''}`}>
          ...
          {minimized?(
            <div className="chat-minimize-button" onClick={() => setMinimized(true)}>
            <span>Minimize</span>
          </div>
          ):(
            <>
            <div className="chat-close-button" onClick={() => setChatVisible(false)}>
              <span>Close</span>
            </div>
            ...
            chatMessages.map((message, index) => (
              <div key={index} className={`chat ${message.role}`}>
                <br />
                <a href={message.link}> <button className='chat-button'>{message.content1}{message.content2}  </button> </a>
              </div>
            ))}
          </div>
