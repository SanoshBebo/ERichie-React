...
const ShopProduct = ({ product }) => {
  ...
  function addProductToCart() {
    dispatch(
      addToCart({
        ...
        imgUrl: product.imageurl.stringValue,
  ...
  function isItSoldOut() {
    ...
    if (product.stock.integerValue <= 0) {
      ...
      tempProduct &&
      tempProduct.quantity >= product.stock.integerValue
    ) {
      return true;
    } else return false;
  }
  ...
  return (
    <div className="d-flex flex-column border rounded m-2 shadow-sm" style={{ width: '162px' }}>
      <div className="d-flex flex-column">
        <ProductImg src={product.imageurl.stringValue} className={'rounded'} style={{ objectFit: 'contain', height: '162px' }} />

        <div className="d-flex justify-content-between px-2 border-top">
          <span className="fw-bold text-wrap text-break">{product.shopname.stringValue}</span>
        </div>
        <NavLink to={`/${product.id}`} className="text-decoration-none mx-2"><small className="text-wrap text-break">{product.productname.stringValue}</small></NavLink>
        <span className="px-2">
          <Rating
      ...
      <div className="d-flex justify-content-between align-items-center p-2 mt-auto">
        <small>
          <span className="fw-bold">Price:</span> ₺{product.price.integerValue}
        </small>
